...
require_once "../config/config_sqli.php";
    ...
    if (isset($_GET['logout'])) {
      ...
      if (isset($_GET['delete'])) {
        ...
        $stmt = "UPDATE room set r_delete='0' WHERE id = $delete_id";
        $stmt = mysqli_query($conn, $stmt);

        if ($stmt) {
            $_SESSION['success'] = "ลบข้อมูลเรียบร้อยแล้ว";
            header("refresh:2; url=../room/show_room.php");
        } 
  }
...
<?php
    ...
    <div class="container bg">
        ...
        <div class=" h4 text-center alert alert-danger mb-4 mt-4" role="alert"> ข้อมูลห้องพัก</div>
        <hr>
        <a href="add_room.php" class="btn btn-success mb-4"><i class="bi bi-plus-circle-fill"></i> เพิ่มข้อมูล</a>
        <a href="restore_room.php" class="btn btn-outline-info mb-4"><i class="bi bi-trash3"></i> คืนค่าข้อมูล</a>
        <table id="datatableid" class="table table-striped table-hover table-bordered">
            ...
            <tbody>
                <?php 
                    ...
                    $stmt = "SELECT * FROM room WHERE r_delete='1'" ;
                    $room=mysqli_query($conn,$stmt);
